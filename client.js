...
parentPort.on('message', msg => {
	...
	if (msg.m === "connect") {
		...
		// forward remote note events so main thread can detect human playing
		if (typeof client.on === 'function') {
			try { client.on('note', m => parentPort.postMessage({ m: 'note', a: m })); } catch (e) { }
			try { client.on('n', m => parentPort.postMessage({ m: 'note', a: m })); } catch (e) { }
		}
