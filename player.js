...
parentPort.on('message', msg => {
	if (msg.m === "load") {
		...
		lastFile = msg.a;
		try { lastDuration = (typeof player.getSongTime === 'function') ? player.getSongTime() : null; } catch (e) { lastDuration = null; }
		parentPort.postMessage({m: "load", id: msg.id, t: end - start, file: lastFile, duration: lastDuration});
	} else if (msg.m === "stop") {
		player.stop();
		parentPort.postMessage({m: "stop", file: lastFile});
	} else if (msg.m === "play") {
		player.play();
		parentPort.postMessage({m: "play", file: lastFile, duration: lastDuration});
	} else if (msg.m === "pause") {
		player.pause();
		parentPort.postMessage({m: "pause", file: lastFile});
	} else if (msg.m === "info") {
		parentPort.postMessage({m: "info", playing: player.isPlaying(), tick: player.tick, total: player.totalTicks, tempo: player.tempo, time: player.getSongTime(), file: lastFile, duration: lastDuration})
	}
})
