...
cl.on('message', msg => {
    ...
    if (msg.m === "hi") {
        cl.postMessage({ m: "userset", name: config.name, color: config.color });
    ...
    setTimeout(() => {
        ...
        for (const [fn, key, vel, noteNumber] of arr) {
            ...
            const clientIndex = config.userkey
                ? Math.floor(keyIndex / (keys.length / client.clients.length))
                : undefined;

            let target;
            // If playing, route all notes to the first token (index 0) so it "plays piano".
            if (lastPlayerInfo.playing && client.clients && client.clients[0]) {
...
const interval = Math.round(2000 - 1950 * ratio);
    ...
    if (nps < 0.1) {
        ...
        client.clients.forEach(c => c.postMessage({ m: "userset", color: "#808080" }));
    } else {
        if (!timer || Math.abs(lastInterval - interval) > 20) {
            if (timer) clearInterval(timer);
            timer = setInterval(() =>
                client.clients.forEach(c => c.postMessage({ m: "userset", color: color.color() })), interval);
...
// custom cursor updater removed — original cursor behavior preserved
...
// === BotStats playing status ===
if (config.botstats && config.botstats.enabled) {
    setInterval(() => {
        try { if (player && player.postMessage) player.postMessage({ m: "info" }); } catch (e) { }

        const state = lastPlayerInfo.state || (lastPlayerInfo.playing ? 'playing' : 'stopped');
        const fileShort = lastPlayerInfo.file ? path.basename(lastPlayerInfo.file) : 'unknown';
        if (state === 'playing') {
            const elapsedRaw = lastPlayerInfo.time || ((Date.now() - (lastPlayerInfo.startedAt || Date.now())) / 1000);
            const totalRaw = lastPlayerInfo.duration || null;
            const elapsedStr = formatPlayerTime(elapsedRaw);
            const totalStr = totalRaw ? formatPlayerTime(totalRaw) : 'unknown';
            const status = `${Math.round(nps)} NPS | ${elapsedStr} / ${totalStr} | Now Playing: ${fileShort}`;
            client.clients.forEach((c, idx) => {
                try {
                    if (idx === 0) c.postMessage({ m: "userset", name: config.name });
                    else c.postMessage({ m: "userset", name: status });
                } catch (e) { }
            });
        } else if (state === 'paused') {
            const status = `Paused: ${fileShort}`;
            client.clients.forEach((c, idx) => {
                try {
                    if (idx === 0) c.postMessage({ m: "userset", name: config.name });
                    else c.postMessage({ m: "userset", name: status });
...
try {
    switch (args[0]) {
        ...
        case "load": {
            ...
            client.clients.forEach(c => c.postMessage({ m: "userset", color: "#FF0000" }));
...
rainbowfun = () => { color.count = (color.count + 1) % config.rainbow.data.length; client.clients.forEach(c => c.postMessage({ m: "userset", color: color.color() })); };
} else if (config.rainbow.mode === "user") { rainbowfun = () => {}; }
...
try {
    ...
    rl.on('line', async (line) => {
        try {
            ...
            line = line.trim();
            if (!line.startsWith(config.prefix)) { rl.prompt(); return; }
